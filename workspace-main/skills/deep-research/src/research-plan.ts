...
export async function analyzeQuery(query: string, isExpanded: boolean = false): Promise<QueryAnalysis> {
  const presetTypes = Object.keys(RESEARCH_ANGLE_PRESETS).join('、');

  const expansionHint = isExpanded
    ? `\n\nIMPORTANT: The query above is a structured research brief generated by the prompt expansion module. Pay special attention to:
- The "任务" (tasks) section: each task should map to one or more research dimensions
- The "输出结构" (output structure) section: the report chapters should align with dimensions
- The "筛选标准" or "拆解层次" sections: these define the depth and focus of each dimension
...
Return the analysis as a structured object.${expansionHint}`,
    schema: z.object({
